...
export function validateProjectionConfig(input: unknown): ProjectionConfig {
    ...
    if ("rotate" in obj && obj.rotate !== undefined) {
        if (Array.isArray(obj.rotate)) {
            ...
            if (rotateArray.length === 3 && rotateArray[0] !== undefined && rotateArray[1] !== undefined && rotateArray[2] !== undefined) {
    ...
    if ("parallels" in obj && obj.parallels !== undefined) {
        if (Array.isArray(obj.parallels)) {
            ...
            if (parallelsArray.length === 2 && parallelsArray[0] !== undefined && parallelsArray[1] !== undefined) {
...
export function sanitizeSVG(svgContent: string): string {
    if (!currentValidationConfig.allowUnsafeContent) {
        ...
        const dangerousAttrs = ["onload", "onerror", "onclick", "onmouseover", "onmouseout", "onfocus", "onblur", "onchange", "onsubmit", "onreset"];
...
export function validateSecurityConfig(input: unknown): Partial<GeographySecurityConfig> {
    const obj = validateObject(input);
    ...
    if ("MAX_RESPONSE_SIZE" in obj && obj.MAX_RESPONSE_SIZE !== undefined) {
        config.MAX_RESPONSE_SIZE = validateNumber(obj.MAX_RESPONSE_SIZE, 1024, 100 * 1024 * 1024);
    }

    if ("ALLOWED_CONTENT_TYPES" in obj && obj.ALLOWED_CONTENT_TYPES !== undefined) {
        config.ALLOWED_CONTENT_TYPES = validateArray(obj.ALLOWED_CONTENT_TYPES, (item) => sanitizeString(item));
    }
...
export function validateStyleObject(input: unknown): Record<string, string | number> {
...
export function validateEventHandler(input: unknown): ((...args: unknown[]) => unknown) | undefined {
    if (input === null || input === undefined) {
        return undefined;
    }
    ...
    const dangerousPatterns = ["eval(", "Function(", "setTimeout(", "setInterval(", "document.write", "innerHTML", "outerHTML", "insertAdjacentHTML"];
    ...
    return input as (...args: unknown[]) => unknown;
}

export function validateComponentProps(props: unknown, allowedProps: readonly string[]): Record<string, unknown> {
