...
export function MapMetadata(props: MapMetadataProps) {
    ...
    return (
        <>
            {/* Basic metadata */}
            <Show when={props.title}><Title>{props.title}</Title></Show>
            <Show when={props.description}><Meta name="description" content={props.description} /></Show>
            <Show when={merged().keywords && merged().keywords!.length > 0}>
                <Meta name="keywords" content={merged().keywords!.join(', ')} />
            </Show>
            <Show when={props.author}><Meta name="author" content={props.author} /></Show>
            <Meta name="viewport" content={merged().viewport} />

            {/* Canonical URL */}
            <Show when={props.canonicalUrl}><Link rel="canonical" href={props.canonicalUrl} /></Show>

            {/* Open Graph metadata */}
            <Show when={props.ogTitle}><Meta property="og:title" content={props.ogTitle} /></Show>
            <Show when={props.ogDescription}><Meta property="og:description" content={props.ogDescription} /></Show>
            <Show when={props.ogImage}><Meta property="og:image" content={props.ogImage} /></Show>
            <Show when={props.ogUrl}><Meta property="og:url" content={props.ogUrl} /></Show>
            <Meta property="og:type" content="website" />

            {/* Twitter Card metadata */}
            <Meta name="twitter:card" content={merged().twitterCard} />
            <Show when={props.twitterTitle}><Meta name="twitter:title" content={props.twitterTitle} /></Show>
            <Show when={props.twitterDescription}><Meta name="twitter:description" content={props.twitterDescription} /></Show>
            <Show when={props.twitterImage}><Meta name="twitter:image" content={props.twitterImage} /></Show>
