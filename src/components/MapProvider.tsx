...
import { validateProjectionConfig, sanitizeString } from "../utils/input-validation";
...
const makeProjection = ({ projectionConfig = {}, projection = "geoEqualEarth", width = 800, height = 600 }: MakeProjectionParams): GeoProjection => {
    ...
    if (!(projectionName in projections)) {
        throw createGeographyError("PROJECTION_ERROR", `Unknown projection: ${sanitizedProjection}`, undefined, {
            availableProjections: Object.keys(projections),
        });
    }

    let proj = (projections[projectionName] as () => GeoProjection)().translate([width / 2, height / 2]);
