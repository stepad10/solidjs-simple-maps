...
export default function ComposableMap(props: ComposableMapProps) {
    ...
    const [local, rest] = splitProps(merged, ["width", "height", "projection", "projectionConfig", "className", "class", "debug", "children"]);

    return (
        <MapProvider width={local.width} height={local.height} projection={local.projection} projectionConfig={local.projectionConfig}>
            <svg viewBox={`0 0 ${local.width} ${local.height}`} class={`rsm-svg ${local.class || ""} ${local.className || ""}`.trim()} {...rest}>
                {local.children}
            </svg>
        </MapProvider>
    );
