import { mergeProps, splitProps, createMemo, JSX, Show } from 'solid-js';
import {
...
export default function ZoomableGroup(props: ZoomableGroupPropsUnion) {
    ...
    const config = createMemo(() => {
        let finalMinZoom = local.minZoom;
        let finalMaxZoom = local.maxZoom;
        ...
        if (isSimpleProps(props)) {
            if (!finalTranslateExtent && local.enablePan !== false) {
                finalTranslateExtent = createTranslateExtent(
