...
describe('Map Elements', () => {
        ...
        const { container } = render(() => (
            <ComposableMap width={800} height={600} projectionConfig={{ scale: 100 }}>
                <Marker coordinates={[0, 0]}>
                    <circle r={5} className="test-marker" />
                </Marker>
            </ComposableMap>
        ));
    ...
    it('renders Line between coordinates', () => {
        const { container } = render(() => (
            <ComposableMap>
                <Line from={[0, 0]} to={[10, 10]} className="test-line" />
            </ComposableMap>
        ));
    ...
    it('renders Graticule', () => {
        const { container } = render(() => (
            <ComposableMap>
                <Graticule className="test-graticule" />
            </ComposableMap>
        ));
    ...
    it('renders Sphere', () => {
        const { container } = render(() => (
            <ComposableMap>
                <Sphere className="test-sphere" id="earth" />
            </ComposableMap>
        ));
